...
class Enformer(snt.Module):
  ...
  def __call__(self, inputs: dict,
    ...
    for ds_input in inputs:
      if ds_input.shape[1]>100000: # Sequence input. This is a kludge for now, TODO: FIX
        trunks.append(self.trunk(ds_input, is_training=is_training))
      else:
        trunks.append(self.side_trunk(ds_input,is_training=is_training))

    trunk_embedding = tf.concat(trunks,
      axis=-1,name='concat')
    return {
