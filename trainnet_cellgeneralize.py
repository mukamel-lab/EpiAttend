...
parser.add_argument('--model_architecture',default=False,type=str,
  choices=['EpiEnformer_SideTrunk','EpiEnformer_TwoStems','EpiEnformer_MLP'],
  help='''
...
def data_wrap(ds: tf.Tensor,
  lags: int = 2,
  ):
  # Given a dataset with size (L,C) create a new dataset
  # of size (L*C,C+1) where the first feature column corresponds to the 
  # current cell type
  C=ds.shape[1]
  ds_outs=[]
  for c in range(C):
    ds0=tf.concat((ds[:,c:c+1],ds),axis=1)
    ds1=[]
    for lag in np.arange(-lags,lags+1):
      ds1.append(tf.roll(ds0,shift=lag,axis=0))
    ds1=tf.concat(ds1,axis=1)
    # TODO: Mask the first and last lag elements 
    ds_outs.append(ds1)
  ds_out=tf.concat(tuple(ds_outs),axis=0)
  return ds_out

def load_tracknames(datadir):
  df=pd.read_csv(f'{datadir}/targets.txt',sep='\t')
  df['cellid']=df['identifier'].str.replace('^.*_C','C',regex=True)
  return df['cellid']

datasets={}
for t in ['train','valid']:
  # Load track names
  common_tracks=set(load_tracknames(args.targets_dir).values)
  for predictor_dir in args.predictors_dirs:
    common_tracks = common_tracks & set(load_tracknames(predictor_dir).values)
  print('Keeping: ', common_tracks)
  ...
  if args.only_sequence:  
    ...
    for predictor_dir in args.predictors_dirs:
      print(predictor_dir)
      keep=load_tracknames(predictor_dir).to_frame().reset_index().set_index('cellid').loc[common_tracks,'index']
      print(tracks.head())
      ds_list.append(me.get_dataset_targets(predictor_dir, t).map(lambda x: tf.gather(x,keep,axis=-1)))
  ...
  keep=load_tracknames(args.targets_dir).to_frame().reset_index().set_index('cellid').loc[common_tracks,'index']
  targets_dataset = me.get_dataset_targets(args.targets_dir, t).map(lambda x: tf.gather(x,keep,axis=-1))

  predictors_datasets=predictors_datasets.map(data_wrap)
  targets_dataset=targets_dataset.map(lambda x: data_wrap(x,lags=0)[:,0:1])
  ...
  datasets[t] = datasets[t].repeat().prefetch(2)
...
print(batch_mc['predictors'].shape, batch_mc['target'].shape)
...
input_shape=(batch_mc['predictors'].shape[-1],)
...
def create_step_function(model, optimizer):
  ...
  def train_step(batch, head, optimizer_clip_norm_global=0.2):
    with tf.GradientTape() as tape:
      outputs = model(batch['predictors'], training=True)
      if type(outputs) is dict:
        outputs=outputs[head]
...
for epoch_i in range(args.epochs):
  ...
  if ((time.time()-t0)>args.validation_interval) or ((global_step-np.max(history['global_step']))>args.validation_steps):
        ...
        for i in tqdm(range(nvalid),desc='Validation'):
            ...
            outputs = model(batch_mc_valid['predictors'], training=False)['target']
